...
class MultiModalityPCA:
    ...
    def _get_pca(self, X, K, update_signal=False):
        """
        ...
        U_K, Lambda_K, V_K = U[:, :K], np.diag(Lambdas[:K]), Vh[:K, :]
        ...
        #print(Lambdas[:K], flush=True)

        if update_signal:
            singval_threshold = 1 + np.sqrt(aspect_ratio)
            if np.min(Lambdas[:K]) < singval_threshold:
                raise ValueError("Signal does not separate from the bulk.")

            # Apply Marčenko-Pastur correction
        ...
        print(f"Sample aligns: {sample_align}")
        print(f"Feature aligns: {feature_align}")
